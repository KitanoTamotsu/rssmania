...
https://user-images.githubusercontent.com/40127279/126052092-d459abff-728d-4de4-a7b7-256f2407c8dd.mp4
...
<br>　
<br>　実装の段階でrssは"rss a"から"rss z"の26種類としたので"+"ではなく、どの文字でもOKとしました
<br>　"rss kkk"みたいな続け打ちができます
### 2.パラメータで表示させる記事の一覧をコントロールを実装する
　出力するJSONフォーマットは、他のRSSを扱ったworkflowと同様に配列を利用して作成するので
<br>　肝はパラメータの文字数（ページ数）と、ループさせる配列の添字を関連づけること
<br>　パラメータの文字数は＄{#1}で取得できることが分かったので、あとは計算だけですね
<br> 
<br>　パラメータの文字数（ページ数）　1、 2、 3、 4、・・・
<br>　各ページのトップ記事の添字　　　0、 9、18、27、・・・
<br>
<br>　終わりの記事は9件分なので9回ループさせればOk
<br>　ループのさせ方は色々あるかと思うので、どのような実装でもかまいません
<br>　
<br>　※bookmarkをAlfredのサーチ対象としている場合rss fで、feed:〜のrssがヒットすることがありましたので
<br>　　意図しないものが表示される可能性があります
<br>　　これらは設定やキーワードで調整することが可能です
### 3.ソースを解釈する（	RSSパース）
...
<br>　ちなみに、Eオプションは拡張正規表現の利用とのことですがなんとなくつけたままです
<br>　下記のようにするとソース内のすべての<title>xxxxxx</title>をスペース区切りで取り出してくれます
<br>　スペース区切りなので配列に格納するのにちょうどよいですね
```
　　grep -oE '<title>[^<]+</title>'
```
<br>　[^<]+というように、『<』以外の文字の連続を指定する部分がミソで、
<br>　例えば任意の文字の連続とすると一番最後の</title>でマッチとなってしまいます（全体で1個）
<br>　
<br>　次にsedで取り出します
```
　　sed 's#<title>\(.*\)</title>#\1#'
...
<br>　内部的な動きがあるのでしょう
<br>　因みにgrepとsedを分ける場合はgをつけたり、Eオプションで正規表現にしたりする必要がありました
<br>　
<br>　最後は配列への格納ですがスペース区切りになっているので、結果をそのまま括弧で括って変数に格納します
```
　　title=(`xxxxx`)
```
...
　RSSの場合は、記事のリンクはlinkタグ、日付はpubDateタグやdc:dateタグで、ある程度ルールがあるようです
...
<br>　titleは文字数が長いとAlfredの出力で…として中略されるので長さを制限しています（フォントにも依存しますが）
...
<br>　　ロジックは同じようなものなのでコピペでつくったのですがピッタリ重なったりすることがあります
<br>　　隠れたオブジェクトが稼働しているとデバッグしにくいです（修正しても直らないという謎の現象が発生します）
...
<br>　　U: 虚構新聞
<br>   　　個人が発信するすべて嘘のニュースサイト。たまに嘘が本当になって誤報の謝罪があるとか
<br>　　V: ダ・ヴィンチニュース　毎日雑学
<br>   　　書籍に関するニュースを発信するサイト。雑学コーナーの一覧ページをパースしてみました
<br>　　W: ついラン
...
<br>　以前は『ダ・ヴィンチニュース　毎日雑学』でしたが4月から更新がないので
<br>
<br>
