...
　各ページのトップ記事の添字　　　0、 9、18、27、・・・
  ...
  ※bashからzshにしたので、トップの添字は 1、10、19、28、・・・になっています
  
　終わりの記事は9件分なので9回ループさせればOk
　ループのさせ方は色々あるかと思うので、どのような実装でもかまいません
　
　※Alfredのサーチ対象の設定で、意図しないものが表示される可能性があります
...
　　　あとはサブタイトルとして記事の日付を入れる配列を用意しました
　
...
　　今回26個のサイトをパースしていますが、curlのオプションはバラバラです。表示できれば気にしないという方針ですので、悪しからず
...
　　つまり、sedの命令は、titleタグ全体をその中身（『.*』）で置き換えるという意味になります
...
　　その後まとめブログアンテナを発見HTMLでパースしてみました
  　"rss k"は急上昇検索キーワードに割り当ててしまっていたので、"rss x"として復活
　　まだ、ちょっと反応遅いけど。。
...
　　もとはFNNプライムオンラインのニュースとしていましたが天気.jpのサプリ記事にしました
　　Fから連想ができませんね。。。
